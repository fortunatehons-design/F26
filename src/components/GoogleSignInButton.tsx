...
export default function GoogleSignInButton({ text = "Continue with Google", className, ...props }: Props) {
  return (
    <button
      type="button"
      className={cn(
        "flex items-center justify-center gap-3 w-full px-4 py-3 bg-white border border-[#dadce0] rounded-md hover:bg-[#f8f9fa] hover:shadow-sm transition-all duration-200 group",
        className
      )}
      {...props}
    >
      ...
      <span className="text-[#3c4043] font-medium text-sm font-sans">
        {text}
      </span>
    </button>
  );
